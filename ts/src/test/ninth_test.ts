...
export default function test(): void {
    console.log("======> NINTH TEST: another check to left and right bruteforce");
    ...
    const tests: Problem[][] = inputs.map(serie => serie.map(input => ({
        ...
        obtainedOutput: solvedWithBranches(input, leftBruteforce, rightBruteforce)
    })));
